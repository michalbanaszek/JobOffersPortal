...
namespace JobOffersPortal.Application.Functions.JobOfferPropositions.Commands.CreateJobOfferProposition
{
    public class CreateJobOfferPropositionCommandHandler : IRequestHandler<CreateJobOfferPropositionCommand, CreateJobOfferPropositionCommandResponse>
    {
        ...
        public CreateJobOfferPropositionCommandHandler(IJobOfferRepository jobOfferRepository, ILogger<CreateJobOfferPropositionCommandHandler> logger, IJobOfferPropositionRepository jobOfferPropositionRepository, IUriService uriService)
        {
        ...
        public async Task<CreateJobOfferPropositionCommandResponse> Handle(CreateJobOfferPropositionCommand request, CancellationToken cancellationToken)
        {
            ...
            var proposition = jobOffer.AddProposition(request.Content, request.JobOfferId);

            await _jobOfferPropositionRepository.AddAsync(proposition);

            _logger.LogInformation("Created JobOfferProposition for JobOffer Id: {0}, Name: {1}", jobOffer.Id, jobOffer.Position);

            var uri = _uriService.Get(proposition.Id, nameof(JobOfferProposition));
